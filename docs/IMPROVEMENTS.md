...
3) [기능] 그래프 구현 및 평균값 모니터링 (1차 완료)
- API: 집계 엔드포인트(`/api/resource-usage/series`) 연동 완료
  - 기간별 버킷(분/시간/일), 이동평균/누적평균 지원
- UI: 시계열 라인 그래프(다중 프록시/지표), 모드 토글, 자동 새로고침
- 다음 단계: 임계치 라인/색상, 헤더 고정, 범례 고도화
