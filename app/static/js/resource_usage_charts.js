...
(function(window) {
    ...
    const ResourceUsageCharts = {
        ...
        /**
         * 이상징후 패널 렌더링 — scaled >= 90인 metric/proxy 조합만 스파크라인 표시
         */
        renderAnomalyPanel() {
            const ru = window.ru;
            if (!window.ApexCharts) return;

            const contentEl = document.getElementById('ruAnomalyContent');
            const statusEl  = document.getElementById('ruAnomalyStatus');
            if (!contentEl || !statusEl) return;

            // 현재 히트맵 셀에서 경고 이상인 것 수집
            const anomalies = [];
            document.querySelectorAll('#ruHeatmapEl .ru-hm-cell[data-scaled]').forEach(cell => {
                const scaled = parseInt(cell.dataset.scaled, 10);
                if (isNaN(scaled) || scaled < 90) return;
                const proxyId  = cell.dataset.proxyId ? parseInt(cell.dataset.proxyId, 10) : null;
                const metricKey = cell.dataset.mk;
                const proxyName = cell.dataset.proxy;
                if (!proxyId || !metricKey) return;
                anomalies.push({ proxyId, metricKey, proxyName, scaled });
            });

            // 기존 이상징후 차트 정리
            if (ru.anomalyCharts) {
                Object.values(ru.anomalyCharts).forEach(c => { try { c.destroy(); } catch(e) {} });
            }
            ru.anomalyCharts = {};

            if (anomalies.length === 0) {
                statusEl.textContent = '이상 없음';
                contentEl.innerHTML = '';
                return;
            }

            statusEl.textContent = `이상징후 ${anomalies.length}건`;

            // 메트릭 레이블 맵
            const metricLabels = {
                cpu: 'CPU', mem: 'MEM', disk: 'DISK', cc: 'Client Count',
                cs: 'Connected Sockets', blocked: 'Blocked',
                httpd: 'HTTP', httpsd: 'HTTPS', http2d: 'HTTP/2'
            };
            ...
            function getBufferKey(mk) {
                return mk === 'httpd' ? 'http' : mk === 'httpsd' ? 'https' : mk === 'http2d' ? 'http2' : mk;
            }

            function formatVal(val, mk) {
                if (val === null || val === undefined) return 'N/A';
                const bk = getBufferKey(mk);
                if (['cpu', 'mem', 'disk'].includes(bk)) return val.toFixed(1) + '%';
                if (['cc', 'cs', 'blocked'].includes(bk)) return val.toLocaleString();
                return val.toFixed(2) + ' Mbps';
            }

            // 스파크라인 색상: 경고(90~110) = 주황, 위험(>110) = 빨강
            function getCardColor(scaled) {
                return scaled > 110 ? '#ef4444' : '#f97316';
            }

            const grid = document.createElement('div');
            grid.className = 'ru-anomaly-grid';
            contentEl.innerHTML = '';
            contentEl.appendChild(grid);

            anomalies.forEach(({ proxyId, metricKey, proxyName, scaled }) => {
                const bk   = getBufferKey(metricKey);
                const data = ((ru.tsBuffer || {})[proxyId] || {})[bk] || [];
                const label = getMetricLabel(metricKey);
                const color = getCardColor(scaled);
                const lastVal = data.length > 0 ? data[data.length - 1].y : null;

                const elId = `ru-anomaly-${proxyId}-${metricKey}`;
                const card = document.createElement('div');
                card.className = 'ru-anomaly-card';
                card.innerHTML =
                    `<div class="ru-anomaly-card-header">` +
                    `<span class="ru-anomaly-proxy" title="${proxyName}">${proxyName}</span>` +
                    `<span class="ru-anomaly-metric" style="color:${color}">${label}</span>` +
                    `<span class="ru-anomaly-value" style="color:${color}">${formatVal(lastVal, metricKey)}</span>` +
                    `</div>` +
                    `<div id="${elId}"></div>`;
                ...
                if (data.length >= 2) {
                    const chart = new ApexCharts(document.getElementById(elId), {
                        ...
                        series: [{ name: label, data: data.slice(-30) }],
                        ...
                        tooltip: {
                            x: { format: 'HH:mm:ss' },
                            y: { formatter: val => val !== null ? formatVal(val, metricKey) : 'N/A' }
                        }
                    });
                    chart.render();
                    ru.anomalyCharts[`${proxyId}-${metricKey}`] = chart;
                } else {
                    document.getElementById(elId).innerHTML =
                        '<p style="font-size:10px;color:#94a3b8;padding:4px 0;">데이터 수집 중...</p>';
                }
            });
        },
        ...
        cleanupAllCharts() {
            ...
            const anomalyContent = document.getElementById('ruAnomalyContent');
            if (anomalyContent) anomalyContent.innerHTML = '';
            const anomalyStatus = document.getElementById('ruAnomalyStatus');
            if (anomalyStatus) anomalyStatus.textContent = '';
        }
    };
