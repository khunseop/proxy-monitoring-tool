...
def get_resource_config(db: Session = Depends(get_db)):
    ...
    if not cfg:
        ...
        db.commit()
        db.refresh(cfg)
    # parse oids and embedded thresholds/interface_oids/interface_thresholds/interface_bandwidths/bandwidth_mbps
    ...
    if isinstance(oids, dict) and isinstance(oids.get('__interface_bandwidths__'), dict):
    ...
    if isinstance(oids, dict):
        oids = {k: v for k, v in oids.items() if k not in ['__thresholds__', '__interface_oids__', '__interface_thresholds__', '__interface_bandwidths__', '__bandwidth_mbps__', '__selected_interfaces__']}
    return ResourceConfigSchema(
        ...
        interface_bandwidths=interface_bandwidths,
...
def update_resource_config(payload: ResourceConfigBase, db: Session = Depends(get_db)):
    ...
    interface_bandwidths_provided = 'interface_bandwidths' in payload_dict
    ...
    interface_bandwidths = payload.interface_bandwidths if interface_bandwidths_provided else {}
    ...
    try:
        ...
        if not interface_bandwidths_provided and isinstance(current, dict) and isinstance(current.get('__interface_bandwidths__'), dict):
            interface_bandwidths = current.get('__interface_bandwidths__') or {}
        ...
        pass
    merged = dict(oids)
    # always embed thresholds, interface_oids, interface_thresholds, interface_bandwidths, and bandwidth_mbps (may be empty/default) to ensure persistence in oids_json
    ...
    merged['__interface_bandwidths__'] = interface_bandwidths
    ...
    if isinstance(oids_out, dict):
        ...
        if '__interface_bandwidths__' in oids_out:
            interface_bandwidths_out = oids_out.get('__interface_bandwidths__') or {}
    ...
    return ResourceConfigSchema(
        ...
        interface_bandwidths=interface_bandwidths_out,
