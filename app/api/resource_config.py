...
def get_resource_config(db: Session = Depends(get_db)):
    ...
    if not cfg:
        ...
        db.commit()
        db.refresh(cfg)
    # parse oids and embedded thresholds/selected_interfaces/bandwidth_mbps
    ...
    if isinstance(oids, dict):
        oids = {k: v for k, v in oids.items() if k not in ['__thresholds__', '__selected_interfaces__', '__bandwidth_mbps__']}
...
def update_resource_config(payload: ResourceConfigBase, db: Session = Depends(get_db)):
    ...
    # Preserve previous thresholds/selected_interfaces/bandwidth_mbps when client doesn't provide them
    try:
        ...
        pass
    merged = dict(oids)
    # always embed thresholds, selected_interfaces, and bandwidth_mbps (may be empty/default) to ensure persistence in oids_json
    ...
    db.commit()
    db.refresh(cfg)
    # Build response splitting embedded thresholds, selected_interfaces, and bandwidth_mbps back out
    ...
    if isinstance(oids_out, dict):
        ...
        oids_out = {k: v for k, v in oids_out.items() if k not in ['__thresholds__', '__selected_interfaces__', '__bandwidth_mbps__']}
    return ResourceConfigSchema(
        id=cfg.id,
        community=cfg.community,
        oids=oids_out,
